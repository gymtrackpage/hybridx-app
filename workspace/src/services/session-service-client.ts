...
export async function linkStravaActivityToSession(sessionId: string, activity: StravaActivity): Promise<void> {
    ...
    if (!snapshot.empty) {
        ...
        const docRef = await addDoc(sessionsCollection, newSessionData);
        sessionRef = docRef;
        sessionData = { id: docRef.id, ...newSessionData, workoutDate: activityDate, startedAt: new Date(activity.start_date) } as WorkoutSession;
    }
   ...
   if (sessionData.workoutDetails) {
        const items = sessionData.workoutDetails.programType === 'running' && sessionData.workoutDetails.runs
